...
| `CIBA_AUTOMATED_DECISION_MODE` | **保留（默认 POST 兼容）**。This selects only the HTTP transport. Authorization is owned by tenant-scoped `CibaDecisionBinding` resources, not a global server secret. |
