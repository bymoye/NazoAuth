...
- 在原生 x86-64 和 Arm64 Linux、Windows、macOS runner 上使用固定 Rust
  toolchain 构建八个平台目标
- 在每个目标上运行 `nazoauthctl` 测试并实际执行两个二进制文件
- 验证应用二进制内嵌的 tag、commit、协议版本和 build ID
...
- GitHub Release 持久制品严格只包含 16 个带平台后缀的可执行文件；JSON、tar、
  bundle、脚本和 SBOM 不属于 Release 制品
- 只在每个既有 Release 制品逐字节一致时恢复部分发布，绝不覆盖 digest 不同的
  tag 或制品
- 除自定义 manifest predicate 外，还生成标准 provenance attestation

独立生产部署通过 `nazoauthctl` 使用这些二进制。生命周期工具先获取制品的
...
- 全部 16 个二进制制品名称与 digest
- 每个平台目标的自定义 ReleaseManifest attestation URL
- 两个内部 SBOM 制品名称与 digest
- Trivy 扫描结果
