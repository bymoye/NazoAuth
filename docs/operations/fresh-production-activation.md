...
3. Install the verified `nazoauthctl` from the same immutable Release and run:
   ...
   Build that material from the fresh suite's public keys, trust anchors,
   credential configuration and callback origins. It contains no secret and
   is not copied from the deleted deployment; all installation secrets and the
   matching local signing identity are generated by the formal install flow.
