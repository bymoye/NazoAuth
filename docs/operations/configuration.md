...
| `CIBA_AUTOMATED_DECISION_MODE` | `disabled` | Automated decisions are closed by default. With a 32+ byte token, `disabled` permits only the OIDF GET/query endpoint for a request-scoped `oidc-fapi-ciba` conformance lease; lease expiry or revocation closes it immediately. Explicit `header` (POST + `Authorization: Bearer`) and `query` (legacy GET/query) modes retain their static transport behavior and are intended only for isolated conformance deployments. |
| `CIBA_AUTOMATED_DECISION_TOKEN` | unset | 32+ byte secret. It is required when `header`/`query` mode is selected and when the default lease-gated GET is used; leaving it unset keeps automated decisions closed. Prefer `CIBA_AUTOMATED_DECISION_TOKEN_FILE`. |
