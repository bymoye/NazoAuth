...
| 新兴与相邻规范 | Attestation-Based Client Authentication、Transaction Tokens、Grant Management、OpenID4VCI/VP、HTTP message signatures 等。 | 已完成入场门禁的能力必须按真实状态描述：OpenID4VCI Issuer、OpenID4VP Verifier、Client Attestation 与 FAPI HTTP Signatures 均为默认关闭/边界化能力；Transaction Tokens 与 Grant Management 仍保持 watchlist。 |
...
| M6 | CIBA / FAPI-CIBA | NI-007 CIBA poll/ping、FAPI-CIBA ID1 四组合兼容、内部强化 profile。 | 官方兼容 profile 与内部强化 profile 文档、metadata 和 conformance 证据隔离。 |
| M7 | 加密响应与可选互操作 | NI-012 UserInfo signing/encryption、NI-013 JARM/JWE。 | cryptographic metadata、per-client key policy 和负向测试完整。 |
| M8 | 草案和相邻生态 | NI-014、NI-015、Browser draft final audit、Attestation、Transaction Tokens、OpenID4VC。 | 已入场能力必须标明“支持；默认关闭 / 实验性支持 / 待最终审计”；未入场能力保持 watchlist，不得混写成当前支持。 |
...
- `crates/authorization-server/tests/unit/bootstrap/cors.rs`
- `crates/authorization-server/tests/unit/http/authorization/*`
...
- `crates/authorization-server/tests/unit/http/profile/*` 或新增 `crates/authorization-server/tests/unit/http/external_login/*`
- `docs/operations/configuration.md`
- `docs/protocol/profile-matrix.md`
...
- [x] **M6-01：补齐 CIBA poll/ping 的产品边界**
...
- OAuth 2.0 Attestation-Based Client Authentication；当前已作为 profile-scoped `attest_jwt_client_auth` 入场，默认关闭，最终 RFC 发布后仍需差异审计。
- Transaction Tokens。
- Grant Management。
...
必须按最终 RFC 复审 challenge/freshness、key binding、trust store、replay、
revocation、DPoP/refresh 交互和 downgrade tests。Transaction Tokens、
Grant Management 和其他未入场候选项继续 deferred，直到各自证据记录中的
re-entry 条件满足。
...
| 当前优先缺口 | M8 治理门禁、RFC 9865 bounded SCIM cursor pagination、RFC 9967 notice-only SET transmitter、Browser-Based Applications draft-27 预发布审计、Client Attestation profile-scoped 实现、FAPI HTTP Signatures 实验性实现、OpenID4VCI Issuer 与 OpenID4VP Verifier 均已完成各自边界内入场；Browser 最终 RFC、Client Attestation 最终 RFC、FAPI HTTP Signatures 稳定规范仍需差异审计，Transaction Tokens 与 Grant Management 保持 deferred 或等待单独产品立项。 |
