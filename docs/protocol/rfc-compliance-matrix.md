...
| PKCE | Require S256 for every authorization-code flow. Plain PKCE and omitted PKCE are rejected for every client type. FAPI clients must generate the challenge per authorization request. | RFC 7636, OAuth 2.1 draft, RFC 9700, FAPI2 | Required with no compatibility exception. | PKCE tests under `crates/authorization-server/tests/unit/http/token/authorization_code/pkce.rs` | No client field, database column, runtime flag, or metadata path may bypass PKCE. |
...
| RFC 7636 | Required everywhere for authorization code. | Implemented | Keep S256 mandatory; plain and omitted PKCE are not implemented. |
