...
状态：动态目录已进入主线；本文记录当前架构约束与剩余验证边界。
...
动态操作直接写 PostgreSQL 的租户目录：创建/更新 binding，或变更 tenant、realm、organization 的 active 状态。目录 mutation 与 revision 更新必须在同一数据库事务中完成；静态租户 JSON 配置入口已经删除。
...
3. 目录管理模式支持 loopback-http 与 trusted-proxy；Direct TLS 尚未实现动态证书/SNI 图，因此启动时明确拒绝。
...
- 固定使用 `DEFAULT_TENANT_ID`，绝不由目录排序或进程配置决定；若没有其 active Host binding，部署级 HTTP control 自然不可路由。
...
`nazoauth migrate` 只维护 schema。首次安装在 server 监听前显式执行 `nazoauth tenant-bootstrap`，把固定系统租户的首条 binding 写入 PostgreSQL；相同输入可安全重放，不同 binding 或已有其他目录历史时失败关闭。revision 仍为 0 时 server 拒绝启动，不存在配置回退。

PostgreSQL 是完整运行期目录；进程配置只提供稳定的非租户 baseline，不决定请求 tenant。当前目录模式不支持 Direct TLS 或 OpenID4VC 的动态 tenant 图；这是明确拒绝而非悄悄降级。
...
4. Actix Host A/B HTTP 测试证明 tenant `Data<T>`、CORS 与 system-only 404；
