ALTER TABLE openid4vp_transactions
    ADD COLUMN create_request_jti VARCHAR(36),
    ADD COLUMN create_request_sha256 VARCHAR(64),
    ADD COLUMN create_request_canonical_json TEXT,
    ...
    ADD CONSTRAINT ck_openid4vp_create_request_shape CHECK (
        (
            create_request_jti IS NULL
            AND create_request_sha256 IS NULL
            AND create_request_canonical_json IS NULL
        )
        OR (
    ...
    ADD CONSTRAINT ck_openid4vp_verification_context_shape CHECK (
        ...
        OR (
            create_request_jti IS NOT NULL
            AND create_request_sha256 IS NOT NULL
            AND create_request_canonical_json IS NOT NULL
            AND verification_run_jti IS NOT NULL
...
ALTER TABLE openid4vp_transactions
    ADD CONSTRAINT uq_openid4vp_transaction_tenant_id UNIQUE (tenant_id, id);
...
CREATE TABLE openid4vp_verification_issuance_jtis (
    ...
    CONSTRAINT fk_openid4vp_verification_issuance_transaction_tenant
...
CREATE INDEX ix_openid4vp_verification_issuance_jtis_transaction
    ON openid4vp_verification_issuance_jtis (tenant_id, transaction_id);

CREATE INDEX ix_openid4vp_cleanup_deadline
...
COMMENT ON COLUMN openid4vp_transactions.create_request_jti IS
    'Caller-generated, tenant-bound management idempotency key for exact OpenID4VP create response recovery; it is non-secret and canonical lowercase UUID text.';
COMMENT ON COLUMN openid4vp_transactions.create_request_canonical_json IS
    'Exact canonical normalized management create request. It is compared together with its SHA-256 digest so a reused JTI cannot alias a different request.';
