...
use crate::{
    ...
    InstanceStateChange, InstanceStateMutation, LifecycleFailure, LifecycleFuture,
    ModuleEventRecord, NoopModuleLifecycle, ReconcileOutcome,
};
...
fn management_view_reports_the_security_profiles_effective_disable_policy() {
    ...
    let views =
        block_on(RuntimeModuleManagement::new(repository, registry, catalog, "instance-a").list())
            .unwrap();
    let ciba = views
        .iter()
        .find(|view| view.module_id == ModuleId::Ciba)
