...
use nazo_identity::{
    ...
    ports::{
        BackupCodeCandidate, EncodedSecretHash, MfaRepositoryPort, MfaTotpKeyRing, RepositoryError,
        RepositoryFuture, TotpCredential, TotpEnrollment, TotpVerificationOutcome,
    },
};
use rand::Rng;

const TOTP_ENVELOPE_VERSION: u8 = 1;
const TOTP_NONCE_LEN: usize = 12;
const TOTP_MIN_PROTECTED_LEN: usize = 1 + TOTP_NONCE_LEN + 16 + 1;
const TOTP_AAD_PREFIX: &[u8] = b"nazo-totp-seed-v1";

#[derive(Clone)]
pub struct MfaRepository {
    pool: DbPool,
    totp_keys: Option<MfaTotpKeyRing>,
}
impl MfaRepository {
    #[must_use]
    ...
    #[must_use]
    pub fn with_totp_key_ring(pool: DbPool, totp_keys: Option<MfaTotpKeyRing>) -> Self {
        Self { pool, totp_keys }
    }
...
fn totp_aad(tenant_id: TenantId, user_id: UserId, key_id: &str) -> Vec<u8> {
