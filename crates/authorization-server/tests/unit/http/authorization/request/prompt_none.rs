...
fn prompt_none_state_with_valkey(valkey: fred::prelude::Client) -> TestInfrastructure {
    let mut settings =
...
async fn live_prompt_none_state() -> Option<TestInfrastructure> {
...
fn unavailable_prompt_none_state() -> TestInfrastructure {
...
fn prompt_none_state_with_database_url(database_url: String) -> TestInfrastructure {
...
async fn prompt_none_consumes_valid_pushed_request_uri_before_issuing_code() {
    ...
    assert_eq!(query.get("state").map(String::as_str), Some("opaque-state"));
    assert_eq!(
        valkey_get(&state.valkey, par_storage_key(&request_uri))
            .await
