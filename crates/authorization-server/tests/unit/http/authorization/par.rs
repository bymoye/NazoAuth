...
fn expanded_par_policy(client: &ClientRow, fapi2: bool) -> ExpandedParAdmissionPolicy<'_> {
...
fn pushed_authorization_request_resources_allow_registered_targets() {
    ...
    let mut params = valid_par_parameters();
    params.insert(
        "resource".to_owned(),
    ...
    assert!(validate_expanded_par_admission(&params, expanded_par_policy(&client, false)).is_ok());
}
...
fn par_policy_does_not_require_request_object_for_dpop_bound_clients() {
    ...
    assert!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &client, "private_key_jwt")
        )
        .is_ok()
    );
}
...
fn par_policy_requires_request_object_when_enabled() {
    ...
    let baseline_client = client(false);
    assert!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &baseline_client, "private_key_jwt")
        )
        .is_ok()
    );
    assert_eq!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &policy_client, "private_key_jwt")
        ),
        Err(ParAdmissionError::RequestObjectRequired)
    );
}
...
#[test]
fn baseline_profile_allows_public_par_client_authentication() {
    let settings = baseline_settings();
    ...
    assert!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &public_client, "none")
        )
        .is_ok()
    );
}
...
fn fapi2_profile_requires_private_key_jwt_or_mtls_client_auth() {
    ...
    assert_eq!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &confidential_client, "client_secret_basic")
        ),
    ...
    assert!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &confidential_client, "private_key_jwt")
        )
        .is_ok()
    );
    assert!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &confidential_client, "tls_client_auth")
        )
        .is_ok()
    );
}
...
fn fapi2_profile_requires_sender_constrained_tokens() {
    ...
    assert_eq!(
        validate_raw_par_admission(
            &HashMap::new(),
            raw_par_policy(&settings, &bearer_client, "private_key_jwt")
        ),
...
fn fapi2_profile_requires_explicit_par_redirect_uri_even_when_unambiguous() {
    ...
    let params = valid_par_parameters();
    assert_eq!(
        validate_expanded_par_admission(&params, expanded_par_policy(&client, true)),
        Err(ParAdmissionError::ExplicitRedirectUriRequired)
    );

    assert!(validate_expanded_par_admission(&params, expanded_par_policy(&client, false)).is_ok());

    let mut with_redirect = params;
    with_redirect.insert(
        "redirect_uri".to_owned(),
        "https://client.example/callback".to_owned(),
    );
    assert!(
        validate_expanded_par_admission(&with_redirect, expanded_par_policy(&client, true)).is_ok()
    );
}
...
fn par_validation_binds_request_uri_to_registered_redirect_uri() {
    let mut params = valid_par_parameters();
    let single_redirect_client = client(false);

    assert!(
        validate_expanded_par_admission(
            &params,
            expanded_par_policy(&single_redirect_client, false)
        )
        .is_ok(),
    ...
    assert_eq!(
        validate_expanded_par_admission(
            &params,
            expanded_par_policy(&multi_redirect_client, false)
        ),
        Err(ParAdmissionError::RedirectUriRequired)
    );
    ...
    assert_eq!(
        ...
        Err(ParAdmissionError::RedirectUriNotRegistered)
    );
...
async fn par_success_persists_request_uri_without_client_secret_material() {
    ...
    assert!(
        !stored.params.contains_key("unknown-extension"),
