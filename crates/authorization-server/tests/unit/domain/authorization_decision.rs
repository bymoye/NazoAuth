...
async fn authorize_decision(
    ...
    req: HttpRequest,
    form: Form<AuthorizationDecisionForm>,
) -> HttpResponse {
    nazo_http_actix::authorize_decision(authorization_decision_endpoint(&state), req, form).await
}

fn authorization_decision_endpoint(
    state: &TestInfrastructure,
) -> Data<AuthorizationDecisionEndpoint> {
...
fn decision_state() -> TestInfrastructure {
    let mut settings =
...
impl DecisionLiveFixture {
    ...
    fn state_with_valkey(&self, valkey: fred::prelude::Client) -> Data<TestInfrastructure> {
