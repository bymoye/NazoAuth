...
fn userinfo_test_state() -> TestInfrastructure {
    let mut settings =
...
fn userinfo_token_service(state: &TestInfrastructure) -> ServerTokenService {
...
async fn live_userinfo_state_from_database_url(
    database_url: String,
...
async fn create_isolated_schema(state: &Data<TestInfrastructure>, schema: &str, tables: &[&str]) {
    exec_sql(
        state,
...
async fn exec_sql(state: &Data<TestInfrastructure>, sql: &str) {
...
async fn drop_schema(state: &Data<TestInfrastructure>, schema: &str) {
    exec_sql(
        state,
...
fn userinfo_state_with_valid_signing_key_invalid_db() -> Data<TestInfrastructure> {
    let key_material = client_signing_fixture(jsonwebtoken::Algorithm::EdDSA);
    let _public_jwk = key_material.public_jwk("userinfo-test-kid");
    let mut settings =
        Settings::from_config(&ConfigSource::default()).expect("default settings should load");
...
async fn live_userinfo_state_with_trusted_proxy() -> Option<Data<TestInfrastructure>> {
...
async fn insert_userinfo_client(state: &Data<TestInfrastructure>, client_id: &str) -> Uuid {
...
fn decode_signed_userinfo(state: &TestInfrastructure, client_id: &str, token: &str) -> Value {
