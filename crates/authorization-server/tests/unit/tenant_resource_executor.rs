...
use crate::tenant_resource_provider::{
    CibaDecisionBindingResourcePayload, IssuedTenantResourceReceipt,
    MtlsTrustAnchorResourcePayload, OauthClientResourcePayload, Openid4vcDatasetResourcePayload,
    Openid4vcTrustPolicyResourcePayload, TenantResourceProviderError, TenantResourceStateSource,
    UserResourcePayload,
};
...
async fn apply_preparation_types_and_sorts_every_ordinary_resource_without_persisting() {
    ...
    let resources = vec![
        ...
        PreparedTenantResource {
            identity: identity(TenantResourceKind::CibaDecisionBinding, "ciba", 4),
            payload: Some(TenantResourcePayload::CibaDecisionBinding(
                CibaDecisionBindingResourcePayload {
                    client_resource_id: "client".to_owned(),
                    user_resource_id: "user".to_owned(),
                    decision_token: "0123456789abcdef0123456789abcdef".to_owned(),
    ...
    assert!(matches!(
        prepared[3],
        PreparedApplyPayload::CibaDecisionBinding(_)
    ));
    assert!(matches!(prepared[4], PreparedApplyPayload::Dataset(_)));
    assert!(matches!(prepared[5], PreparedApplyPayload::TrustPolicy(_)));
...
async fn postgres_executor_applies_and_revokes_the_complete_ordinary_resource_graph() {
    ...
    let ciba = identity(TenantResourceKind::CibaDecisionBinding, "ordinary-ciba", 5);
    let dataset = identity(TenantResourceKind::Openid4vcDataset, "ordinary-dataset", 6);
    let resources = vec![
        PreparedTenantResource {
