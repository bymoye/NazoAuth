...
def main() -> None:
    ...
    expected_policy_keys = {
        "schema",
        "database_rollback_compatible",
        "minimum_supported_version",
        "migration_floor",
        "rationale",
    }
    if set(policy) != expected_policy_keys or policy["schema"] != 1:
        raise SystemExit("release update policy has an unexpected schema")
    if not isinstance(policy["database_rollback_compatible"], bool):
        raise SystemExit("database_rollback_compatible must be boolean")
    if not re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+", policy["minimum_supported_version"]):
    ...
    manifest = {
        ...
        "rollback": {
            "database_compatible": policy["database_rollback_compatible"],
